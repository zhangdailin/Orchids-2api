...
func TestPuterRefreshNeverDeletesModelsMissingFromOneCatalogSnapshot(t *testing.T) {
	for _, source := range []string{
		"puter_public_models_live_probe",
		"puter_public_models_unverified",
		"puter_public_models_quota_limited",
	} {
		if shouldDeleteMissingModelsOnRefresh("Puter", source) {
			t.Fatalf("source %q must not delete Puter models from a single catalog snapshot", source)
		}
	}
}

func TestPuterRefreshVerificationReflectsSource(t *testing.T) {
	if !modelRefreshCandidatesVerified("Puter", "puter_public_models_live_probe") {
		t.Fatal("successful live-probe candidates should be verified")
	}
	for _, source := range []string{"puter_public_models_unverified", "puter_public_models_quota_limited"} {
		if modelRefreshCandidatesVerified("Puter", source) {
			t.Fatalf("source %q must remain unverified", source)
		}
	}
	if !modelRefreshCandidatesVerified("Warp", "feature_model_choice_all") {
		t.Fatal("non-Puter refresh verification behavior changed")
	}
}

func TestApplyPuterRefreshKeepsUnverifiedFreeModelsInMaintenance(t *testing.T) {
	s, cleanup := setupModelRefreshStore(t)
	defer cleanup()

	ctx := context.Background()
	clearModelsForChannel(t, ctx, s, "Puter")
	modelID := "infron:deepseek/deepseek-v4-flash:free"
	candidates := []discoveredModel{{ID: modelID, Name: "DeepSeek V4 Flash (Free)", SortOrder: 0}}

	result, err := applyModelRefresh(ctx, s, "Puter", "puter_public_models_quota_limited", candidates)
	if err != nil {
		t.Fatalf("applyModelRefresh(quota_limited) error = %v", err)
	}
	if result.Verified != 0 || result.Added != 1 || len(result.OfflineModelIDs) != 1 {
		t.Fatalf("result=%+v want one unverified maintenance model", result)
	}
	model, err := s.GetModelByChannelAndModelID(ctx, "Puter", modelID)
	if err != nil {
		t.Fatalf("GetModelByChannelAndModelID() error = %v", err)
	}
	if model.Status != store.ModelStatusMaintenance || model.Verified {
		t.Fatalf("model=%+v want maintenance and unverified", model)
	}

	result, err = applyModelRefresh(ctx, s, "Puter", "puter_public_models_live_probe", candidates)
	if err != nil {
		t.Fatalf("applyModelRefresh(live_probe) error = %v", err)
	}
	if result.Verified != 1 || result.Updated != 1 {
		t.Fatalf("result=%+v want verified=1 updated=1", result)
	}
	model, err = s.GetModelByChannelAndModelID(ctx, "Puter", modelID)
	if err != nil {
		t.Fatalf("GetModelByChannelAndModelID() error = %v", err)
	}
	if model.Status != store.ModelStatusAvailable || !model.Verified {
		t.Fatalf("model=%+v want available and verified", model)
	}
}
