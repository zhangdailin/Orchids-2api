...
func TestReconcileDiscoveredModelsPrunesOnlyProviderScope(t *testing.T) {
	mini := miniredis.RunT(t)
	s, err := New(Options{StoreMode: "redis", RedisAddr: mini.Addr(), RedisPrefix: "scope:"})
	if err != nil {
		t.Fatal(err)
	}
	t.Cleanup(func() { _ = s.Close(); mini.Close() })
	ctx := context.Background()
	rows := []*Model{
		{Channel: "Grok", ModelID: "grok-4.7", Provider: "web", Origin: "catalog", Status: ModelStatusAvailable},
		{Channel: "Grok", ModelID: "grok-4.6", Provider: "build", Origin: "catalog", Status: ModelStatusAvailable},
		{Channel: "Grok", ModelID: "stale-build", Provider: "build", Origin: "manual", Status: ModelStatusAvailable},
		{Channel: "Grok", ModelID: "console-model", Provider: "console", Origin: "discovery", Status: ModelStatusAvailable},
	}
	...
	result, err := s.ReconcileDiscoveredModels(ctx, "Grok", []*Model{{Channel: "Grok", ModelID: "grok-4.7", Provider: "build", Status: ModelStatusAvailable, Verified: true}}, ModelReconcileOptions{Prune: true, ProviderScope: "build"})
	if err != nil {
		t.Fatal(err)
	}
	if result.Deleted != 2 {
	...
	got, err := s.GetModelByChannelAndModelID(ctx, "Grok", "grok-4.7")
	if err != nil {
		t.Fatal(err)
	}
	if got.Provider != "build" || !got.Verified {
		t.Fatalf("matching admin row was not replaced by upstream truth: %+v", got)
	}
	if _, err := s.GetModelByChannelAndModelID(ctx, "Grok", "console-model"); err != nil {
		t.Fatalf("console scope was pruned: %v", err)
	}
}
