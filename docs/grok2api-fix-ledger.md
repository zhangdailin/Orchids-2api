...
真正的未修条目共 **6 条**：3 条非 P3（A9-1 属新功能、A3-7/A6-1 属整块移植），3 条 P3（改动需连同对外管理契约/前端一起排期）。其余早期列在本节的条目已全部关闭或归入"有意保留"，见下方两张记录表。
...
| A4-15 | P3 | 管理端模型接口形状：分页信封 / 分组 / 同步端点。改动需连同管理前端一起做，属对外管理契约 |
| A4-17 | P3 | Codex 未知模型 `description` 文案不同：客户端不解析该字段，纯文案 |
| A6-14 | P3 | 订阅/等级推断阈值不同：需要 auto/fast 两套窗口形态的数据模型才能正确改写，凭猜测改阈值比现状更糟 |
...
4. **A4-15**（P3，管理契约）：模型接口分页信封/分组/同步端点，需连同管理前端一次提交。
5. **A4-17 / A6-14**（P3，产品决策）：文案与等级推断阈值，前者随时可改，后者需要 auto/fast 两套窗口的数据模型。
...
| 已修复 | 149 | P0 ×5、P1 ×61、P2 ×66、P3 ×17 | 见第一、五～十一节各表 |
| 有意保留 | 16 | P0 ×0、P1 ×6、P2 ×9、P3 ×1 | `A1-11`、`A2-1`、`A2-6`、`A3-8`、`A3-9`、`A4-1`、`A4-2`、`A4-3`、`A4-11`、`A5-2`、`A5-4`、`A5-5`、`A5-7`、`A5-42`、`A7-2`、`A8-12` |
| 未修 | 6 | P0 ×1、P1 ×2、P3 ×3 | `A9-1`、`A3-7`、`A6-1`、`A4-15`、`A4-17`、`A6-14` |

149 + 16 + 6 = 171；严重度合计 6 + 69 + 75 + 21 = 171，无重复计数、无遗漏。

- **P0 未修 1 条**：`A9-1` 计费层（新功能，非缺陷回归）。
- **P1 未修 2 条**：`A3-7` 网关侧 compaction（整块移植）、`A6-1` 质量降级重试的流式 hold 缓冲（部分完成）。
- **P3 未修 3 条**：`A4-15`、`A4-17`、`A6-14`。
- 第一节列的旧"未修清单"中，`A7-7`、`A7-8`、`A7-10`、`A5-29`、`A6-2/A6-4`、`A6-12/A6-13`、`A2-2`、`A5-24/A5-25/A5-27/A5-40`、`A3-13`、`A4-5/A4-6`、`A4-16`、`A3-14`、`A6-9`、`A9-13`、`A1-7/A1-8/A1-10`、`A3-1/A3-4/A3-6` 均已关闭，记录保留在第三节"已解决的旧条目"表。

验证（文档改动不涉及代码路径；代码侧状态为最后一次全量校验）：

- `go build ./...`、`go vet ./...`、`go test ./... -count=1` 全绿（第八轮结束时）。
- `python3 docs/grok2api-audit/recount.py` 退出码 0，输出 171 / 149 / 16 / 6。
- 本轮仅修改本台账与审计报告抬头，新增复算脚本，未触碰代码。
